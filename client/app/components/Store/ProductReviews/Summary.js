      ...
      {averageRating && (
        <div className='d-flex flex-wrap align-items-center mt-2'>
          <ReactStars
            ...
            value={averageRating}
          />
          {totalReviews > 0 && <span>на основе {totalReviews} отзывов.</span>}
        </div>
      )}
      ...
      {totalReviews > 0 ? (
        ratingSummary.map((r, obj) => (
          <div key={obj} className='d-flex align-items-center mb-2'>
            <div className='left'>
              <span>{parseInt(Object.keys(r)[0])} звезда</span>
            </div>
        ...
        <NotFound>
          ...
          <p className='mb-2'>Будьте первым, кто добавит отзыв.</p>
        </NotFound>
      )}
    </div>
  );
